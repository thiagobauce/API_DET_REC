...
class PartialFC_V2(torch.nn.Module):
    ...
    def sample(self, labels, index_positive):
        ...
        with torch.no_grad():
            ...
            if self.num_sample - positive.size(0) >= 0:
                perm = torch.rand(size=[self.num_local]).to(device)
                perm[positive] = 2.0
    ...
    def forward(
        ...
        _gather_embeddings = [
            torch.zeros((batch_size, self.embedding_size)).to(device)
            for _ in range(self.world_size)
        ]
        _gather_labels = [
            torch.zeros(batch_size).long().to(device) for _ in range(self.world_size)
        ]
